...
export default function AdminBlogDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (!blogData) {
      postService.getPostById(blogId)
        .then(res => {
          const b = res?.data || res || null;
          setBlog(b);
        })
        .catch(() => Alert.alert('Error', 'Cannot load blog.'))
  ...
  if (!blog) {
    return (
      <View style={styles.loading}>
        <Text style={{ color: '#636e72' }}>Blog not found.</Text>
      </View>
    );
  }

  const hasImg = blog.blogImgUrl && blog.blogImgUrl.startsWith('http');
  const createdAt = blog.created_at ? new Date(blog.created_at).toLocaleString('vi-VN') : '—';
  const updatedAt = blog.updated_at ? new Date(blog.updated_at).toLocaleString('vi-VN') : '—';
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 48 }}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle} numberOfLines={1}>{blog.title}</Text>
      ...
      {/* Cover Image */}
      {hasImg ? (
        <Image source={{ uri: blog.blogImgUrl }} style={styles.coverImage} resizeMode="cover" />
      ) : (
...
const styles = StyleSheet.create({
  ...
  body: { padding: 20 },
  title: { fontSize: 24, fontWeight: '800', color: '#2d3436', marginBottom: 10 },
  ...
  infoCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 24 },
  ...
  infoValue: { fontSize: 14, color: '#2d3436', fontWeight: '600', flex: 1, textAlign: 'right', marginLeft: 8 },
  deleteBtn: {
