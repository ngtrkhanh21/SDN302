...
const DEFAULT_AVATAR = 'https://ui-avatars.com/api/?name=User&background=00b894&color=fff';
...
export default function PostManageScreen({ navigation }) {
  ...
  useEffect(() => { loadPosts(); }, []);
  useEffect(() => { applySearch(posts, search); }, [search]);
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.88}
        onPress={() => navigation.navigate('AdminBlogDetail', { blogId: item._id, blogData: item })}
      >
        {hasImg && (
          <Image source={{ uri: item.blogImgUrl }} style={styles.cardImage} resizeMode="cover" />
        )}
        <View style={styles.cardBody}>
          <Text style={styles.postTitle} numberOfLines={2}>{item.title || 'Untitled'}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {isLoading ? (
        <ActivityIndicator color="#00b894" size="large" style={{ marginTop: 32 }} />
      ) : (
        <FlatList
          data={filtered}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#55efc4', paddingTop: 56, paddingHorizontal: 18 },
  ...
  cardBody: { padding: 16 },
  postTitle: { fontSize: 17, fontWeight: '800', color: '#2d3436', marginBottom: 6 },
  postContent: { fontSize: 14, color: '#636e72', lineHeight: 20, marginBottom: 12 },
  cardFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  postDate: { fontSize: 12, color: '#b2bec3' },
  deleteBtn: { backgroundColor: '#d63031', paddingHorizontal: 14, paddingVertical: 7, borderRadius: 10 },
  deleteBtnText: { color: '#fff', fontSize: 13, fontWeight: '700' },
  emptyText: { textAlign: 'center', color: '#2d3436', marginTop: 40, fontSize: 15 },
});
