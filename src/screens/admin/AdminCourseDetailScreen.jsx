...
export default function AdminCourseDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (!courseData) {
      ...
      courseService.getCourseDetail(courseId)
        ...
        .catch(() => Alert.alert('Error', 'Cannot load course.'))
  ...
  const handleRemove = () => {
    Alert.alert('Remove Course', `Remove "${course?.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Removed', 'Course has been removed.', [
              { text: 'OK', onPress: () => navigation.goBack() },
            ]);
          } catch {
            Alert.alert('Error', 'Cannot remove course.');
          }
        },
      },
    ]);
  ...
  const avgRating = reviews.length
    ? (reviews.reduce((s, r) => s + (r.rating || r.star || 0), 0) / reviews.length).toFixed(1)
    : null;
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 48 }}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topBarTitle} numberOfLines={1}>{course.name || 'Course Detail'}</Text>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.titleRow}>
          ...
          <View style={[styles.badge, { backgroundColor: course.status === 'published' ? '#d1fae5' : '#fef3c7' }]}>
            <Text style={[styles.badgeText, { color: course.status === 'published' ? '#059669' : '#d97706' }]}>
              {course.status}
            </Text>
          </View>
        </View>
          ...
          {course.price > 0 ? (
            <>
              <View style={styles.stat}>
                <Text style={styles.statValue}>{Number(course.price).toLocaleString('vi-VN')}đ</Text>
              ...
              {course.discount > 0 && (
                <>
                  ...
                  <View style={styles.stat}>
                    <Text style={[styles.statValue, { color: '#d63031' }]}>-{course.discount}%</Text>
            ...
            <View style={styles.stat}>
              <Text style={[styles.statValue, { color: '#059669' }]}>Free</Text>
        ...
        <View style={styles.tagsRow}>
          ...
          <View style={[styles.tag, { backgroundColor: (RISK_COLORS[risk] || '#00b894') + '25' }]}>
        ...
        <Text style={styles.description}>{course.content || 'Chưa có mô tả.'}</Text>

        {/* Gallery */}
        {course.imageUrls && course.imageUrls.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>Hình ảnh ({course.imageUrls.length})</Text>
        ...
        <View style={styles.infoCard}>
          <InfoRow label="Ngày tạo" value={course.created_at ? new Date(course.created_at).toLocaleString('vi-VN') : '—'} />
          <InfoRow label="Cập nhật" value={course.updated_at ? new Date(course.updated_at).toLocaleString('vi-VN') : '—'} last />
        </View>
            ...
            {reviews.slice(0, 5).map((r, i) => (
              <View key={String(r._id || i)} style={styles.reviewCard}>
                <View style={styles.reviewHeader}>
                  <Text style={styles.reviewerName}>{r.user?.name || r.user_name || 'Ẩn danh'}</Text>
                  <Text style={{ fontSize: 13 }}>{'⭐'.repeat(r.rating || r.star || 0)}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 56, paddingBottom: 12, backgroundColor: '#fff' },
  backBtn: { padding: 8, marginRight: 8 },
  topBarTitle: { flex: 1, fontSize: 18, fontWeight: '700', color: '#2d3436' },
  removeHeaderBtn: { padding: 8 },
  coverImage: { width: '100%', height: 220 },
  coverPlaceholder: { width: '100%', height: 160, backgroundColor: '#dfe6e9', alignItems: 'center', justifyContent: 'center' },
  body: { padding: 20 },
  titleRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 16, gap: 8 },
  ...
  statsRow: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 16, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 },
  ...
  sectionTitle: { fontSize: 15, fontWeight: '700', color: '#2d3436', marginBottom: 10, marginTop: 8 },
  description: { fontSize: 15, color: '#636e72', lineHeight: 22, marginBottom: 20 },
  galleryImage: { width: 140, height: 100, borderRadius: 12, marginRight: 10, backgroundColor: '#dfe6e9' },
  videoList: { backgroundColor: '#fff', borderRadius: 14, padding: 12, marginBottom: 20 },
  videoItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f1f2f6' },
  ...
  infoValue: { fontSize: 14, color: '#2d3436', fontWeight: '600', textAlign: 'right', flex: 1, marginLeft: 8 },
  ...
  removeBtn: { backgroundColor: '#d63031', borderRadius: 16, paddingVertical: 16, alignItems: 'center', marginTop: 24 },
