...
const ROLE_MAP = {
  0: { label: 'Admin', color: '#d63031', bg: '#ffe8e6' },
  1: { label: 'Staff', color: '#0984e3', bg: '#e3f2fd' },
  2: { label: 'Customer', color: '#00b894', bg: '#e8f8f5' },
  3: { label: 'Instructor', color: '#6c5ce7', bg: '#f0edff' },
};

const VERIFY_MAP = {
  0: { label: 'Unverified', color: '#fdcb6e' },
  1: { label: 'Verified', color: '#00b894' },
};
...
export default function GlobalAccountManageScreen() {
  ...
  const loadUsers = async () => {
    try {
      ...
      Alert.alert('Error', error.response?.data?.message || 'Cannot load users.');
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
  ...
  const renderUser = ({ item }) => {
    const role = ROLE_MAP[item.role] || { label: 'Unknown', color: '#636e72', bg: '#f1f2f6' };
    const verify = VERIFY_MAP[item.verify] || { label: 'Unknown', color: '#b2bec3' };
    ...
    return (
      <View style={styles.card}>
        <View style={styles.cardTop}>
          ...
          <View style={styles.userInfo}>
            <Text style={styles.userName} numberOfLines={1}>
              {item.name || item.username || 'No Name'}
            </Text>
            <Text style={styles.userEmail} numberOfLines={1}>{item.email}</Text>
            {dob && <Text style={styles.userMeta}>🎂 {dob}</Text>}
            {item.location ? <Text style={styles.userMeta}>📍 {item.location}</Text> : null}
          </View>
        </View>
        ...
        <View style={styles.badgeRow}>
          ...
          {item.bio ? (
            <View style={[styles.badge, { backgroundColor: '#f1f2f6' }]}>
              <Text style={[styles.badgeText, { color: '#636e72' }]} numberOfLines={1}>
                {item.bio.slice(0, 24)}
              </Text>
            </View>
          ) : null}
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>{filtered.length} user(s) found</Text>
      ...
      <View style={styles.filterRow}>
        {roleFilters.map(f => (
          <TouchableOpacity
            key={String(f.value)}
            style={[styles.filterChip, selectedRole === f.value && styles.filterChipActive]}
            onPress={() => setSelectedRole(f.value)}
          >
            <Text style={[styles.filterChipText, selectedRole === f.value && styles.filterChipTextActive]}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {isLoading ? (
        <ActivityIndicator color="#e17055" size="large" style={{ marginTop: 32 }} />
      ) : (
        <FlatList
          data={filtered}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#ffeaa7', paddingTop: 56, paddingHorizontal: 20 },
  ...
  userInfo: { flex: 1 },
  userName: { fontSize: 16, fontWeight: '700', color: '#2d3436', marginBottom: 2 },
