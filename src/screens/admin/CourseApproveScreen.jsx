...
export default function CourseApproveScreen({ navigation }) {
  ...
  useEffect(() => { applySearch(courses, search); }, [search]);

  const onRefresh = () => { setRefreshing(true); loadCourses(); };

  const handleDelete = course => {
    Alert.alert('Remove Course', `Remove "${course.name || course.title}"?`, [
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.85}
        onPress={() => navigation.navigate('AdminCourseDetail', { courseId, courseName: title, courseData: item })}
      >
        {/* Image */}
        {hasImage ? (
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.tagsRow}>
            <View style={[styles.tag, { backgroundColor: (AUDIENCE_COLORS[audience] || '#6c5ce7') + '20' }]}>
              <Text style={[styles.tagText, { color: AUDIENCE_COLORS[audience] || '#6c5ce7' }]}>👥 {audience}</Text>
            </View>
            <View style={[styles.tag, { backgroundColor: (RISK_COLORS[risk] || '#00b894') + '20' }]}>
              <Text style={[styles.tagText, { color: RISK_COLORS[risk] || '#00b894' }]}>⚠️ {risk}</Text>
            </View>
          </View>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Course Moderation</Text>
      <Text style={styles.subtitle}>{filtered.length} course(s) · Tap to view details</Text>
      ...
      {isLoading ? (
        <ActivityIndicator color="#0984e3" size="large" style={{ marginTop: 32 }} />
      ) : (
        <FlatList
          data={filtered}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#74b9ff', paddingTop: 56, paddingHorizontal: 20 },
  ...
  courseTitle: { flex: 1, fontSize: 16, fontWeight: '700', color: '#2d3436', marginRight: 8 },
  ...
  emptyText: { textAlign: 'center', color: '#dfe6e9', marginTop: 40, fontSize: 15 },
});
